...
""")    #för att fixa kanterna så när man trycker på random background tar den upp hela skärmen.
...
with ui.column().classes("relative w-full h-screen items-center justify-center") as main_container:
    ...
    ui.button("10 000 000 pts", on_click=lambda: victory(10000000)) #skulle kunna göra så att när man trycker på denna så ändras den vita bakgrunden till nån annan image

with ui.column().classes("absolute left-0 top-0"):
    ui.button("+1 gain (costs 1 pt)", on_click=lambda: gains(1, 1)).classes("text-1xl w-30 h-30 absolute left-0 top-0")
    ui.button("+10 gain (costs 10 pts)", on_click=lambda: gains(10, 10)).classes("text-1xl w-30 h-30 absolute left-0 top-31")
    ui.button("+100 gain (costs 100 pts)", on_click=lambda: gains(100, 100)).classes("text-1xl w-30 h-30 absolute left-0 top-62")
    ui.button("+1 000 gain (costs 1 000 pts)", on_click=lambda: gains(1000, 1000)).classes("text-1xl w-30 h-30 absolute left-0 top-93")
    ui.button("+10 000 gain (costs 10 000 pts)", on_click=lambda: gains(10000, 10000)).classes("text-1xl w-30 h-30 absolute left-0 top-124")
